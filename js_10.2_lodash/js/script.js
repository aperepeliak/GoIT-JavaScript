...
// Создание одноуровнего массива скиллов
var flatten = _.flattenDeep(mapSkills);

// Сортировка
var sorted = _.sortBy(flatten);

// Удаление повторяющихся скиллов
var sortedUnique = _.sortedUniq(sorted);

// Все выражение одной строкой
var uniqSkills = _.sortedUniq(_.sortBy(_.flattenDeep(_.map(objArray, 'skills'))));
console.log('uniqSkills = ', uniqSkills);

// 2)
// Выборка по двум полям объектов, для удобства. Загуглено: https://github.com/lodash/lodash/issues/1247
var mapNames = _.map(objArray, _.partial(_.ary(_.pick, 2), _, ['name', 'friends']));

// Сортировка по количеству друзей
var mapNamesSorted = _.sortBy(objArray, 'friends');

// Создание массива имен
var namesSortedByFriends = _.map(mapNamesSorted, 'name');
console.log('namesSortedByFriends = ', namesSortedByFriends);

// 3)
var allFriends = _.uniq(_.map(_.flattenDeep(_.map(objArray, 'friends')), 'name'));
